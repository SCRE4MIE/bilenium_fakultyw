...
class WalkNotificationCountView(generics.GenericAPIView):
    """
    Get walk notifications count.
    ...
    def get(self, request, *args, **kwargs):  # noqa: D102
        try:
            ...
            if len(walk_notifi_customuser) == 0:
                return Response(status=status.HTTP_404_NOT_FOUND)
            return Response({'count': len(walk_notifi_customuser)}, status=status.HTTP_200_OK)
        except WalkNotifiCustomUser.DoesNotExist:
            return Response(status=status.HTTP_400_BAD_REQUEST)
